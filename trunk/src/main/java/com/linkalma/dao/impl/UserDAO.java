...
public class UserDAO implements IUserDAO
{
	...
	public long createUser(final User alumni)
	{
		...
		getJdbcTemplateObject().update(new PreparedStatementCreator()
		{
			...
			{
				PreparedStatement ps = connection.prepareStatement(QueryConstants.INSERT_USER_QUERY, new String[]
				{ "id" });
				...
				ps.setDate(20, null); // CreateDTTM - Purposely set to null, so
										// that it picks up default value from
										// CREATE Table script
	...
	public int createCredentials(User alumni)
	{
		...
		return getJdbcTemplateObject().update(QueryConstants.INSERT_USER_CREDENTIALS_QUERY, alumni.getUserID(), alumni.getEmailAddress(),
				Cipher.DIGEST_PASSWORD(alumni.getPassword()));
	...
	public User getUserProfile(final User alumni)
	{
		...
		User user = getJdbcTemplateObject().queryForObject(QueryConstants.SELECT_USER_DETAILS, new Object[]
		{ alumni.getUserID() }, new UserMapper());
		return user;
	}
	...
	public User getUserWorkplace(User alumni)
	{
		...
		List<UserWorkplaceDTO> userWorkplaceList = getJdbcTemplateObject().query(QueryConstants.GET_USER_WORKPLACE_DETAILS, new Object[]
		{ alumni.getUserID() }, new UserWorkplaceMapper());
	...
	{
		int count = getJdbcTemplateObject().queryForInt(QueryConstants.CHECK_USER_QUERY, new Object[]
		{ emailAddress });
		if (count > 0)
			return true;
		else
	...
	public List<User> getfriendSuggestions(User alumni, String notificationType)
	{
		...
		{
			userList = getJdbcTemplateObject().query(QueryConstants.GET_PENDING_ALUMNUS_REQUEST, new Object[]
			{ alumni.getEmailAddress() }, new UserSuggestionMapper());

		}
		else if (ApplicationConstants.FRIEND_SUGGESTIONS.equalsIgnoreCase(notificationType))
		{
			userList = getJdbcTemplateObject().query(QueryConstants.GET_ALUMNUS_SUGGESTION, new Object[]
			{ alumni.getUserID(), alumni.getUserID() }, new UserSuggestionMapper());
		}
		else if (ApplicationConstants.FRIEND_REQUEST_ACCEPTED.equalsIgnoreCase(notificationType))
		{
			userList = getJdbcTemplateObject().query(QueryConstants.GET_MY_CONNECTED_ALUMNUS, new Object[]
			{ alumni.getEmailAddress(), alumni.getEmailAddress(), alumni.getEmailAddress() }, new UserSuggestionMapper());
		}
